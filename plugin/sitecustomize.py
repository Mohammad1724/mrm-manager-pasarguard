"""Persistent MRM bootstrap for PasarGuard.
...
after the process has already imported `app.routers`, which leaves the MRM
UI visible while `/api/mrm/*` returns 404.

The installer adds this directory to PYTHONPATH. Python imports `sitecustomize`
before executing `main.py`, so the MRM router is registered deterministically
...
BOOTSTRAP_LOG = Path("/var/lib/pasarguard/mrm/bootstrap.log")
PROFILE_ROUTE = "/api/mrm/profile"


def _log(message: str) -> None:
    line = f"[MRM bootstrap] {message}\n"
    try:
...
def _bootstrap() -> None:
    ...
    try:
        ...
        backend = importlib.import_module("mrm_admin_subscriptions")
        ...
        if not added or not _has_profile_route(added):
            raise RuntimeError("MRM router did not expose the profile route")

(() => {
  ...
  const css = `
    ...
    #${ROOT_ID} .z-appearance-card{border-color:color-mix(in srgb,#14ADA6 22%,hsl(var(--border)));background:linear-gradient(145deg,hsl(var(--card)),color-mix(in srgb,hsl(var(--card)) 94%,#14ADA6 6%))}
    ...
    #${ROOT_ID} .z-appearance-apply{border:0;border-radius:var(--radius,.5rem);padding:.58rem .78rem;color:#fff;background:linear-gradient(135deg,#0E8F8A,#0B6E6A);font:inherit;font-size:.7rem;font-weight:850;cursor:pointer}
  ...
  function renderUpdateNotice() {
    ...
    node.innerHTML = `<div class="z-un-text"><strong>نسخه جدید MRM منتشر شده</strong>از Settings → MRM می‌توانید بروزرسانی را مستقیم از پنل انجام دهید.</div><button type="button">باز کردن MRM</button>`;

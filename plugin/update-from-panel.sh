...
if [[ ! -x "${MRM_CLI}" ]]; then write_status failed "MRM CLI was not found on the host" yes "$target"; rm -f "${REQUEST_FILE}"; exit 1; fi
set +e; "${MRM_CLI}" update >"${LOG_FILE}" 2>&1; rc=$?; set -e; chmod 600 "${LOG_FILE}" || true; rm -f "${REQUEST_FILE}"
if [[ $rc -eq 0 ]]; then write_status success "MRM update completed successfully" yes "$target"; exit 0; fi
write_status failed "MRM update failed; see panel-update.log" yes "$target"; exit "$rc"

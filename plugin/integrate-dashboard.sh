...
(() => {{
  try {{
    const key='theme', backupKey='mrm-pasarguard-theme';
...
tag=f'<script id="{marker}" src="/statics/mrm-special.js?v={version}" defer></script>'
...
patch_router_container() {
  ...
  docker cp "${BACKEND_PY}" "${cid}:${target_dir}/mrm_admin_subscriptions.py" >/dev/null
...
integrate_host_dashboard() {
  ...
  if ! cmp -s "${ADMIN_JS}" "${build_dir}/statics/mrm-special.js" 2>/dev/null; then install -m 0644 "${ADMIN_JS}" "${build_dir}/statics/mrm-special.js"; fi
...
integrate_docker() {
  ...
  if [[ -n "${build_dir}" ]]; then
    ...
    docker cp "${ADMIN_JS}" "${cid}:${build_dir}/statics/mrm-special.js" >/dev/null

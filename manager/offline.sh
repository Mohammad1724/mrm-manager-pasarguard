...
# Managed by MRM Iran/Offline Mode v1.3.1
...
offline_show_status() {
    ...
    clear
    ui_header "IRAN / OFFLINE MODE v1.3.1"
...
offline_apply_recommended_apt() {
    local BACKUP_DIR
    clear
    ui_header "APPLY IRAN APT MIRROR v1.3.1"
    if ! offline_require_ubuntu; then
        ui_error "This module currently supports Ubuntu only"
        pause
        return
...
offline_apply_both_recommended() {
    local BACKUP_DIR
    clear
    ui_header "APPLY IRAN MIRRORS v1.3.1"
    if ! offline_require_ubuntu; then
        ui_error "This module currently supports Ubuntu only"
        pause
        return

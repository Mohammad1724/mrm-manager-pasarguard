...
mrm_render_home_dashboard() {
    ...
    ui_section "HOME DASHBOARD - $(get_mrm_version 2>/dev/null || echo v1.3.1)"
...
run_full_diagnostics() {
    ...
    clear
    detect_active_panel > /dev/null
    ui_header "DOCTOR - FULL SYSTEM DIAGNOSTICS v1.3.1"
...
run_doctor_cli() {
    local MODE="${1:-full}"
    detect_active_panel > /dev/null
    echo "=== MRM DOCTOR v1.3.1"
    echo "Date: $(date)"
    echo "Version: $(get_mrm_version 2>/dev/null || echo v1.3.1)"
...
diagnostics_menu() {
    while true; do
        clear
        ui_header "DIAGNOSTICS & DOCTOR v1.3.1"
        mrm_render_home_dashboard

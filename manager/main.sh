...
if [[ "$1" == "--version" || "$1" == "-v" ]]; then
    ...
    echo "MRM Manager ${MRM_VERSION:-$(cat /opt/mrm-manager/VERSION 2>/dev/null || echo 1.4.9)}"
    exit 0
fi
...
panel_menu() {
    while true; do
        clear
        echo "=== 🎛️ PANEL CONTROL v${MRM_VERSION:-1.4.9} ==="
        echo ""
...
tools_menu() {
    while true; do
        clear
        echo "=== 🛠️ TOOLS v${MRM_VERSION:-1.4.9} ==="
        echo ""
...
main_menu() {
    ...
    while true; do
        clear
        local VER="${MRM_VERSION:-$(cat /opt/mrm-manager/VERSION 2>/dev/null || echo "1.4.9")}"
        echo "╔══════════════════════════════════════════════╗"

...
echo -e "${CYAN}║      Fix: interactive launch & syntax errors ║${NC}"
echo -e "${CYAN}╚══════════════════════════════════════════════╝${NC}"
echo ""
...
rm -f \
    "$INSTALL_DIR/site.sh" \
    "$INSTALL_DIR/port_manager.sh" \
    "$INSTALL_DIR/migrator.sh" \
    2>/dev/null
...
curl -sL -f \
    -o "$INSTALL_DIR/index.html" \
    "$REPO_BASE_URL/templates/subscription/index.html" \
    2>/dev/null \
...
if [[ "$1" == "--version" || "$1" == "-v" ]]; then
    echo "MRM Manager $(cat /opt/mrm-manager/VERSION 2>/dev/null || echo 1.0.4)"
    exit 0
fi
...
echo -e "${CYAN}Type 'mrm' to run MRM Manager${NC}"
echo ""

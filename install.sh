#!/bin/bash
# MRM Manager Installer v1.0.3 - No hang at all
...
echo -e "${CYAN}╔══════════════════════════════════════════════╗${NC}"
echo -e "${CYAN}║      MRM Manager Installer v1.0.3            ║${NC}"
echo -e "${CYAN}║      Fix: no hang on mrm command             ║${NC}"
echo -e "${CYAN}╚══════════════════════════════════════════════╝${NC}"
echo ""
...
echo -e "${BLUE}[2/4] Installing core files v1.0.3...${NC}"
...
echo ""
echo -e "${GREEN}✔ MRM Manager v1.0.3 installed${NC}"
...
echo ""
if [[ "$RUN_NOW" =~ ^[Yy]$ ]]; then
    echo -e "${GREEN}Run: mrm${NC}"
    # Don't auto-run to avoid hang in installer, just tell user
    echo -e "${YELLOW}Please run 'mrm' manually to start${NC}"
else
